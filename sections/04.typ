...
где $R$ --- *оператор примитвной рекурсии*.

Простыми словами, началльное значение $f(..., 0) = g(...)$, а все последующие определяются значением другой функции для предыдущего: $f(..., y + 1) = h(..., y)$.
